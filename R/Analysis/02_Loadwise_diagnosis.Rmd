...
names(loadwise_results_kappa) <- c('196', '392', '-392', 'Test')

names(loadwise_results_acc) <- c('196', '392', '-392', 'Test')

diag_final_f_loadwise_acc <- loadwise_results_acc %>% 
                                tidyr::gather(., key = 'Load [N]', value = 'Accuracy', -Test ) %>%  
                                ggplot(., aes(x = Test, y = Accuracy, fill = `Load [N]`)) + 
                                  geom_boxplot() +
                                  xlab('') + 
                                  theme_minimal() +
                                  coord_flip() + 
...
diag_final_f_loadwise_kappa <- loadwise_results_kappa %>% 
                                tidyr::gather(., key = 'Load [N]', value = 'Kappa', -Test ) %>%  
                                ggplot(., aes(x = Test, y = Kappa, fill = `Load [N]`)) + 
                                  geom_boxplot() +
                                  xlab('') + 
                                  theme_minimal() +
                                  coord_flip() + 
...
diag_final_f_loadwise %>%
  cowplot::save_plot(plot = .,
                     filename = here::here('figures', 'diag_final_f_loadwise_boxplot.pdf'),
                     dpi = 450,
                     bg = 'transparent'
                     )
